...
<head>
    ...
    <script>
        ...
        $(() => {
            gfn_setSortTh("zombiePlayRecordList", "selectZombiePlayRecordList(1)");
            ...
            selectZombiePlayRecordList(1);
        });

        const selectZombiePlayRecordList = pageNo => {
            let comAjax = new ComAjax("zombiePlayRecordForm");
            comAjax.setUrl("<c:url value='/selectZombiePlayRecordList' />");
            comAjax.setCallback("selectZombiePlayRecordListCallback");
            comAjax.addParam("pageIndex", pageNo);
            comAjax.addParam("pageRow", 5);
            comAjax.addParam("orderBy", $('#zombiePlayRecordListCurOrderBy').val());
            comAjax.ajax();
        };

        const selectZombiePlayRecordListCallback = data => {
            let cnt = data.map.cnt;
            let body = $("#zombiePlayRecordListTbl>tbody");
            body.empty();
            let str = "";
            if (cnt == 0) {
                ...
                let params = {
                    divId: "zombiePlayRecordListPageNav",
                    pageIndex: "pageIndex",
                    totalCount: cnt,
                    eventName: "selectZombiePlayRecordList",
                    recordCount: 5
                };
        ...
        const openCreatePlayModal = () => {
            createPlayModal.open(GAME.ZOMBIE);
        }
    </script>
</head>
        ...
        <div class="row">
            <div class="col-xl-12 mb-5 mb-xl-0">
                <div class="card shadow mt-5">
                    ...
                    <div class="card-body">
                        <form id="zombiePlayRecordForm" onsubmit="return false;">
                            <input type="hidden" id="zombiePlayRecordListCurOrderBy">
                        ...
                        <div class="table-responsive">
                            <table class="table align-items-center table-flush" id="zombiePlayRecordListTbl">
                                <thead class="thead-light">
                                <tr>
                                    <th name="zombiePlayRecordListSortTh" id="sortTh_playNm">
                                        플레이이름 <span name="zombiePlayRecordListSort"
                                                    id="zombiePlayRecordListSort_playNm"
                                                    class="fa"></span>
                                    </th>
                                    <th name="zombiePlayRecordListSortTh" id="sortTh_gameNm">
                                        게임 <span name="zombiePlayRecordListSort" id="zombiePlayRecordListSort_gameNm"
                                                 class="fa"></span>
                                    </th>
                                    <th scope="col">모임</th>
                    ...
                    <div class="card-footer py-4">
                        <nav aria-label="">
                            <ul class="pagination pagination-sm justify-content-end mb-0"
                                id="zombiePlayRecordListPageNav"></ul>
                        </nav>
                    </div>
                </div>
            </div>
